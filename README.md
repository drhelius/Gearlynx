...
- **Sprite Bounding Box**: Homebrew and debug builds can draw outlines after Suzy finishes rendering sprites, using the selected Lynx pen number. The Debug menu can draw all sprites or only SCBs with `SPRCOLL` bit 7 (`$80`) set. Real Lynx hardware ignores this bit, but production builds should clear it.
