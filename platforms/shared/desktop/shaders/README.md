...
Gearlynx desktop shader presets use `.gshader` files. A preset is an INI-style file that describes one or more GLSL fragment shader passes. Presets placed in this `shaders` directory are discovered once when the desktop app starts and appear in the `Video > Shader` combo after `Pixel Perfect`.

GLSL files are normal fragment shaders. Gearlynx supplies a fullscreen vertex shader and prepends the correct GLSL version, so shader files should not include a `#version` line.
...
The final pass is what Gearlynx displays. Non-final passes render to intermediate textures.
